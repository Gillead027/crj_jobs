...
function ElegantTextSection({
  ...
  return (
    <section className="mt-7">
      ...
      <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a5b44]">
        {title}
      </h3>

      {/* Este parágrafo mostra dado real ou sugestão. */}
      <p className="mt-3 text-sm leading-7 text-[#3b332e]">
...
export function ElegantResumeTemplate({ resume }: ResumeTemplateProps) {
  ...
  return (
    <article className="mx-auto aspect-[210/297] min-h-[980px] w-full max-w-[794px] rounded-lg bg-[#f4eee6] p-7 text-[#2f2a27] shadow-2xl shadow-stone-300/50 sm:p-12">
      {/* Este cabeçalho coloca nome grande e objetivo em itálico no topo. */}
      <header className="border-b border-[#d6c3b2] pb-9">
        {/* Este nome grande usa terracota, como no modelo clássico de referência. */}
        <h2 className="text-4xl font-bold leading-tight tracking-normal text-[#8f4f38] sm:text-6xl">
          {displayedName}
        </h2>

        {/* Este objetivo fica logo abaixo do nome em itálico. */}
        <p className="mt-3 text-lg italic text-[#6f5a4d]">{displayedObjective}</p>
      </header>

      {/* Este bloco cria a estrutura de duas colunas do currículo. */}
      <div className="grid gap-10 pt-9 md:grid-cols-[0.68fr_1.32fr]">
        {/* Esta coluna menor guarda contato, habilidades e cursos. */}
        <aside className="border-[#d8c7b9] md:border-r md:pr-8">
          ...
          <section>
            ...
            <dl className="mt-4 space-y-4 text-sm leading-6">
              <div>
                ...
                <dd>{resume.phone || <EmptyText>A preencher</EmptyText>}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd>{resume.city || <EmptyText>A preencher</EmptyText>}</dd>
              </div>
              <div>
                ...
                <dd>{resume.age || <EmptyText>A preencher</EmptyText>}</dd>
              </div>
            </dl>
          </section>
